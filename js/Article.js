...
HeaderNavigation()
/*Star Rate*/
const _Star = document.querySelectorAll(".Rate_star_svg")
let AreyouRate=false;
_Star.forEach((e, index) => {
    e.addEventListener("click", () => {
    
        for (let i = 0; i <5 ; i++) {
            _Star[i].classList.remove("Rate_Active_Star");
            
        }
        for (let i = 0; i <index+1 ; i++) {
            _Star[i].classList.add("Rate_Active_Star");
            AreyouRate=true;
        }
